...
function Register(props) {
    ...
    const [nameIncorrect, setNameIncorrect] = React.useState(false);
        ...
        <div className="register">
            <div className="register__container">
                ...
                <form className="register__form" onSubmit={handleSubmit}>
                    ...
                    name="name"
                    type="text"
                    className={`register__input ${(nameIncorrect && nameError) ? 'register__input_type_error' : ''}`}
                    placeholder="Max"
                    autoComplete="off"
                    minLength="2"
                    maxLength="30"
                    ...
                    onBlur={blurHandler}
                    />
                    { (nameIncorrect && nameError) && <span id="register-input-error" className="register__input-error">{nameError}</span> }
                    <p className="register__input-name">E-mail</p>
                    <input
                    id="input-user-email"
                    name="email"
                    type="email"
                    className={`register__input ${(emailIncorrect && emailError) ? 'register__input_type_error' : ''}`}
                    ...
                    onBlur={blurHandler}
                    />
                    { (emailIncorrect && emailError) && <span id="register-input-error" className="register__input-error">{emailError}</span> }
                    <p className="register__input-name">Пароль</p>
                    <input
                    id="input-user-password"
                    name="password"
                    type="password"
                    className={`register__input ${(passwordIncorrect && passwordError) ? 'register__input_type_error' : ''}`}
                    ...
                    onBlur={blurHandler}
                    />
                    { (passwordIncorrect && passwordError) && <span id="register-input-error" className="register__input-error">{passwordError}</span> }
